...
import bus_finder
from dbus_connection import DBusConnection, DBusConnectionError
...
class OMXPlayer(object):
    ...
    """
    def __init__(self, filename, args=[], bus_address_finder=None, Connection=None, cleaner=FileCleaner('/tmp/*omxplayer*')):
        ...
        if not bus_address_finder:
            bus_address_finder = bus_finder.BusFinder()
        if not Connection:
        ...
        self.connection = self._setup_dbus_connection(Connection, bus_address_finder)
    ...
    def _run_omxplayer(self, filename, devnull):
        ...
        process = subprocess.Popen(command, stdout=devnull, preexec_fn=os.setsid)
        self._process_monitor = threading.Thread(target=monitor, args=(process, on_exit))
